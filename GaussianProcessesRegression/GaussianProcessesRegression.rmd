...
Using caTools we split the dglist column of the counts for each sequence and split it 70/30. The GauPro_Kernel_model is created using the training sets. Each time this is run the kernel used to create the Gaussian model is different, resulting in a different graph each time.
...
Each time the kernel will change and produce a different graph. We could run this multiple times to find the averages between them. This will show that the code is indeed using a kernel for the function generator.
