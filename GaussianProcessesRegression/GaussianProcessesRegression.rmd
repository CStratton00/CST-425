...
```{r covariance}
#Will be very self explainatory, just take what is calculated above for gaussian processes and put it in cov().
# Note: Define x somewhere

#kxx <- cov(x,x)
#kxy <- cov(x,y)

#GauPro_kernel_model$pred_var(x, kxx, kxy)
```
...
## Posterior Noise-Free Observations and Fitting Predictions to Noisy Observations in Gaussian Processes
Noise is the measurement of random variation in a graph that describes unexpected plot results from an expected function. Gaussian processes utilize noise to generate the most accurate model for a dataset based on a mean and covariance function. Using Bayesian statistics, a Gaussian Process generates functions using the mean and covariance function for a set of posterior observations through the fixed number of points. Since the generated functions must always pass through this set of given points, the Gaussian Process model can start by assuming that all of the observations are perfectly known and noise-free. However, when attempting to create accurately predicting functions with a Gaussian process, it must make a new assumption that a set of constraining observations is not perfectly known. Thus, it assumes that these constraining observations must be some level of normally-distributed noise. Thus, any function that the Gaussian process generates must fit the posterior noise-free observations to a set of constrained noisy observations. By using and comparing a sample of functions generated through this process, the Gaussian process can return an accurate regression line of the observations.
...
https://2020blogfor.github.io/posts/2020/04/kf_gp/
https://www.r-bloggers.com/2012/04/gaussian-process-regression-with-r/