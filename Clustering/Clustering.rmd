...
kmeansdf <- z_scores[!rowSums(z_scores>3), ]
kmeansdf <- data.frame(kmeansdf)
```
...
associationdf <- read.csv("C:/Users/Evan/OneDrive/Documents/GitHub/CST-425/Clustering/Store_Hackathon_Ideal_Data.csv")
associationdf <- data.frame(associationdf)
head(associationdf)
```
...
# Data cleaning
associationdf <- associationdf[,4:8]
head(associationdf)
associationdf <- na.omit(associationdf) # Get rid of missing values
associationdf <- scale(associationdf) # Set scale for data
head(associationdf, n = 3)

# Detect outliers using z-score method
z_scores <- as.data.frame(sapply(associationdf, function(associationdf) (abs(associationdf-mean(associationdf))/sd(associationdf))))

# Keep rows in dataframe with all z-scores less than absolute value of 3
associationdf <- z_scores[!rowSums(z_scores>3), ]
```
