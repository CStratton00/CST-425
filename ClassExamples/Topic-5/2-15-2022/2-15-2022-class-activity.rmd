---
title: "PCA - Team 1 Class Activity"
...
pca <- read.csv(file="C:/Users/andre/Documents/GitHub/CST-425/ClassExamples/Topic-5/2-15-2022/PCA_stock.csv", header=TRUE,sep=",")
...
pcscores <- .94*AnnualReturn + .85*ExcessReturn + .00*AbsWinRate + .20*SystematicRisk + .73*TotalRisk + .63*RelWinRate
...
```{r p_at_0-100_scale}
scoreMax <- max(pcscores)
scoreMin <- min(pcscores)
s <- 100/(scoreMax - (-scoreMin))
m <- 0 - (scoreMin * s)
pcscaled <- m + s*pcscores
round(pcscaled,2)
```
...
1. PCA was performed with 6 variables: AnnualReturn,ExcessReturn,SystematicRisk,TotalRisk,AbsWinRate,RelWinRate
...
4. PCA revealed that variables 1, 2, and 5 explain 88%, 72%, and 53% of the variance respectively (review table above and Cronbach's alpha)